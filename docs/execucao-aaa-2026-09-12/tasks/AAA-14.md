# AAA-14 — Tornar builds e gates reproduzíveis
...
Status: PLANNED | Responsável: plataforma build | Fase: 2 | Risco: R2 | Pontos: 5

## Objetivo

Cada pacote executa build/lint/typecheck real quando aplicável; nenhuma tarefa echo simula controle.

## Contexto e dependências

Achados: A12
Áreas: 1, 14, 15
Predecessores: AAA-02, AAA-15, AAA-06
Contratos: C08
Exclusividade do repositório: sim
Locks de recursos: dependency-graph, isolated-db:AAA-14, ports:AAA-14
...
- Cada pacote executa build/lint/typecheck real quando aplicável; nenhuma tarefa echo simula controle.
- Remover tsc || true e passWithNoTests de suítes obrigatórias; avisos seguem uma política única documentada.
- Todos Dockerfiles usam lock congelado e runtime suportado definido; prod resolve as versões auditadas.
- Mudanças abrangentes só nos manifests/config/Dockerfiles; não corrigir lógica de domínio neste cartão.
...
- `pnpm install --frozen-lockfile` — ambiente: isolated checkout
- `pnpm exec turbo run lint typecheck build --dry=json` — ambiente: local
