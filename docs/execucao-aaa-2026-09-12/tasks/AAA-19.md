# AAA-19 — Vincular autoria de notas à identidade autenticada
...
Status: PLANNED | Responsável: backend notas | Fase: 1 | Risco: R2 | Pontos: 3

## Objetivo

Autor padrão vem da sessão e ignora/rejeita spoof de authorId.
...
Locks de recursos: isolated-db:AAA-19, ports:AAA-19
...
- modules/notes
- apps/desk-api/src/__tests__/aaa-19.integration.test.ts
...
- Autor padrão vem da sessão e ignora/rejeita spoof de authorId.
- Delegação só existe se explicitamente definida e autorizada; histórico preserva ator e autor de forma verdadeira.
- Teste HTTP A escreve tentando atribuir a B e prova que autoria não é falsificada; leitura respeita recurso.
...
- `pnpm --filter @cvg/notes test` — ambiente: isolated

### A criar

- `pnpm --filter @cvg/desk-api exec vitest run src/__tests__/aaa-19.integration.test.ts` — ambiente: isolated
