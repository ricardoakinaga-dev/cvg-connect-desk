...
def render(data, notes):
    tasks=data['tasks']
    index=['# Backlog de produção', '', '**Fonte canônica:** [BACKLOG.json](BACKLOG.json). Revisão R2: preserve as entregas descritas em cada cartão. Status deste ciclo começa PLANNED; checks NOT_RUN/TO_CREATE exigem descoberta e execução segura. As evidências recebidas são históricas.', '', '| Tarefa | Prioridade | Marco | Pontos restantes | Dependências |', '|---|---|---|---|---|']
    for t in tasks:
        tid=t['id']
    ...
    lines += ['', '## Achados da auditoria de entregas', '', '| Achado | Tarefas responsáveis |', '|---|---|']
    findings=json.loads((BASE/data['findings_source']).read_text())
    for f in findings:
        owners=[t for t in tasks if f['id'] in t['audit_findings']]
        lines.append('| '+f['id']+' — '+f['title']+' | '+', '.join(f"[{t['id']}](tasks/{t['id']}.md)" for t in owners)+' |')
