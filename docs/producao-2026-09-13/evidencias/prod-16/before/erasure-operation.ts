...
import { loadContactGraph, normalizeScope, type InventoryScope, type ContactGraph } from './contact-graph';
...
export type RunErasureResult =
  ...
  | { ok: false; reason: 'not_found' | 'out_of_scope' | 'not_resumable' };
...
export async function runContactErasure(input: RunErasureInput): Promise<RunErasureResult> {
  ...
  try {
    ...
    // Corrida no mesmo requestId: devolve a operação vencedora.
    if ((error as { code?: string }).code === '23505') {
      const winner = await findOperationByRequestId(requestId);
      if (winner) return { ok: true, report: { ...reportFromRow(winner), deduplicated: true } };
    }
    throw error;
  }
...
export async function resumeContactErasure(operationId: string): Promise<RunErasureResult> {
  const [row] = await db
    .select()
    .from(schema.privacyOperations)
    .where(eq(schema.privacyOperations.id, operationId))
  ...
  const effectivePolicy: PrivacyPolicy = { ...policy, mode };
  const scope = JSON.parse(row.scope || '{}') as InventoryScope;
  const graph = await loadContactGraph(row.contactId, scope);
  if (!graph) return { ok: false, reason: 'not_found' };
  const context = buildContext(graph);
...
export async function getPrivacyOperation(operationId: string): Promise<ErasureReport | null> {
  const [row] = await db
    .select()
    .from(schema.privacyOperations)
    .where(eq(schema.privacyOperations.id, operationId))
