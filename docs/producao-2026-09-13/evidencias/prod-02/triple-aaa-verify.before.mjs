...
run('runtime', "node -e \"const v=process.versions.node; if (!v.startsWith('24.')) { console.error('expected Node 24.x, got ' + v); process.exit(1); } console.log('node ' + v);\"");
run('install', 'pnpm install --frozen-lockfile');
run('lint', 'pnpm lint');
run('typecheck', 'pnpm typecheck');
run('unit', 'pnpm test:ci', DB_ENV);
run('postgres-real', 'pnpm test:postgres-real', DB_ENV);
run('migration-check', 'pnpm --filter @cvg/database db:check', DB_ENV);
run('build', 'pnpm build');

for (const [name, command] of [
  ['security-audit', 'pnpm security:audit'],
  ['security-audit-prod', 'pnpm security:audit:prod'],
]) {
  try {
    execSync(command, { cwd: root, stdio: 'pipe', timeout: 300000 });
...
// Executa sem pipe: o exit code do vitest (thresholds 85/80/85/85 em
// packages/shared/vitest.config.ts) precisa reprovar o gate; o pipe anterior
// deixava só o status do `grep` contar.
try {
  const coverageOut = execSync('pnpm --filter @cvg/shared exec vitest run --coverage', {
    cwd: root,
    stdio: 'pipe',
    timeout: 300000,
  }).toString();
  if (!coverageOut.includes('All files')) throw new Error('relatório de cobertura ausente na saída');
...
const lockfileSha256 = createHash('sha256').update(readFileSync(join(root, 'pnpm-lock.yaml'))).digest('hex');
...
const report = {
  ...
  tools: { node: process.version, pnpm: execSync('pnpm -v').toString().trim(), turbo: execSync('pnpm exec turbo --version').toString().trim() },
  gates,
  aaaStatuses: aaa,
  final,
