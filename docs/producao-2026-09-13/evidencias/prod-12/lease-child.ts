/**
 * PROD-12 — processo RPC de lease entre processos REAIS.
 *
 * Executa UMA ação (claim/renew/ack/nack) usando a MESMA porta de produção
 * (`ConsumerAwareOutboxReader` + `PostgresOutboxLease` de @cvg/events) e
 * imprime o resultado em uma linha JSON. O orquestrador (suíte prod-12) lança
 * vários destes processos simultaneamente para provar exclusão mútua, renewal,
 * fencing por owner+generation e takeover entre processos distintos.
 *
 * Uso: PROD12_LEASE_ACTION='{"cmd":"claim",...}' tsx lease-child.ts
 */
import {
  CONSUMER_IDS,
...
type Action =
  | {
      ...
      limit?: number;
      eventTypes?: string[];
      /** Barreira opcional: só reivindica neste instante (epoch ms). */
  ...
  switch (action.cmd) {
    ...
    case 'renew': {
      ...
      console.log(JSON.stringify({
        ok: true,
        cmd: 'renew',
        renewed: renewed ? serializeLease(renewed) : null,
      }));
      break;
    }
    case 'ack': {
