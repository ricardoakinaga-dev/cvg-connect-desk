...
def render(data, notes):
    tasks=data['tasks']
    index=['# Backlog de produção', '', '**Fonte canônica:** [BACKLOG.json](BACKLOG.json). Todos os cartões iniciam PLANNED. Checks TO_CREATE são especificações propostas, não comandos já implementados ou executados. Ownership precisa ser confirmado na descoberta.', '', '| Tarefa | Prioridade | Marco | Pontos | Dependências |', '|---|---|---|---|---|']
    for t in tasks:
        tid=t['id']
...
def main():
    ...
    audit=BASE.parent/'auditorias/2026-09-13'
    assert hashlib.sha256((audit/'RELATORIO.md').read_bytes()).hexdigest()==data['audit_sha256'], 'audit hash mismatch'
    assert hashlib.sha256((audit/'notas.json').read_bytes()).hexdigest()==data['notes_sha256'], 'notes hash mismatch'
    notes=json.loads((audit/'notas.json').read_text())
