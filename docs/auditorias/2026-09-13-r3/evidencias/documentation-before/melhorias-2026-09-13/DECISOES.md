# Decisões para continuidade — revisão R2

D01–D06 permanecem abertas segundo o registro recebido. Nesta solicitação estão autorizadas auditoria e atualização documental; a implementação futura depende de solicitação própria. A aprovação de condições operacionais não substitui autorização vinculada ao pacote de implantação.
...
- **Recomendação (técnica):** implementar N:N compatível (`tutor_patients` com FK composta e unicidade), migrando o 1:N atual com backfill idempotente e leitura dupla durante rollout. Proposta completa em `../producao-2026-09-13/evidencias/prod-06/D03-PROPOSTA.md`.
