...
export async function registerAlertRoutes(app: FastifyInstance) {
  app.post(
    ...
    async (request, reply) => {
      try {
        ...
        const body = request.body as CreateAlertBody;
  ...
  app.get('/alerts', { preHandler: [authenticate, requirePermission('alerts:read')] }, async (request, reply) => {
    try {
      const query = request.query as { status?: string; severity?: string; type?: string };
  ...
  app.get(
    ...
    async (request, reply) => {
      try {
        ...
        const alert = await alertRepository.findById(params.id);
        if (!alert) {
