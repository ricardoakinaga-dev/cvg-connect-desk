...
async function deliverOutbound(
  ...
  if (result.success) {
    const finalized = await outboundDeliveryRepository.finalizeDelivery(ids.deliveryId, ids.messageId, {
      status: 'sent',
      providerMessageId: result.messageId,
    });
    // PROD-11/AC2: a resposta reflete o estado FINAL persistido — um callback
    // de reconciliação concorrente pode já ter terminalizado a intenção.
    if (finalized.delivery) {
      console.log(`[deliverOutbound] Mensagem aceita pelo gateway: ${result.messageId}`);
      return {
        outcome: deriveOutboundOutcome(finalized.delivery, finalized.message),
        status: finalized.message?.status ?? 'sent',
      };
    }
  ...
  if (result.failureKind === 'definitive') {
    ...
    if (finalized.delivery) {
      ...
      return {
        outcome: deriveOutboundOutcome(finalized.delivery, finalized.message),
        status: finalized.message?.status ?? 'failed',
      };
    }
  ...
  if (finalized.delivery) {
    ...
    return {
      outcome: deriveOutboundOutcome(finalized.delivery, finalized.message),
      status: finalized.message?.status ?? 'pending',
    };
  }
